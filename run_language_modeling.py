...
class TextDataset(Dataset):
    def __init__(self, file_path: str, args, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            for i, sample in enumerate(x):
                ...
                for res_id in self.responses[-1]:
                    if res_id in self.kn_vocs[-1]:
                        choose.append(0)
                    else: choose.append(1)
                self.chooses.append(choose)

            pickle.dump((self.histories, self.responses, self.segments, self.knowledges, self.kn_vocs, self.chooses),
                        open(cached_features_file, "wb"))
    ...
    def __getitem__(self, item):
        return self.histories[item], self.responses[item], self.knowledges[item], self.kn_vocs[item], \
               self.segments[item], self.chooses[item]
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    return TextDataset(file_path=file_path, args=args, block_size=args.block_size)
...
def collate(example):
    ...
    pad_chooses = max([cal_max_len(c_inst) for c_inst in example[:, 5]])
    ...
    chooses = [pad_data(c_inst, pad_chooses) for c_inst in example[:, 5]]

    return histories, responses, kns, kn_vocs, segments, chooses
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            chooses = torch.cat([-100 * torch.ones([histories.shape[0], histories.shape[-1]]).float().to(args.device), chooses], 1)
            lm_labels = torch.cat([-100 * torch.ones([histories.shape[0], histories.shape[-1]]).long().to(args.device), responses], 1)
            lm_labels[lm_labels == 0] = -100

            outputs, x_kn_att = model(input_ids=(histories, responses, knowledges, kn_vocs, chooses),
                                        lm_labels=lm_labels, token_type_ids=segments,
                                        use_posterior=False, use_bow=args.use_bow)
            loss = outputs[1]
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    result = {"perplexity": perplexity, "cls loss": cls_loss, "bow loss": bow_loss, "kl loss": kl_loss}
...
def get_emo_voc(tokenizer):
    voc = []
    with open(r"./tests_samples/emotion_voc/negative-words.txt", encoding='utf-8') as f:
        voc += [word.strip() for word in f.readlines()[31:]]

    with open(r"./tests_samples/emotion_voc/positive-words.txt", encoding='utf-8') as f:
        voc += [word.strip() for word in f.readlines()[30:]]

    voc_ids = []
    for word in voc:
        for id in tokenizer.encode(word):
            if len(tokenizer.decode(id)) <= 3: continue
            voc_ids.append(id)

    voc_ids = list(set(voc_ids))
    # for id in voc_ids:
    #     print(tokenizer.decode(id))
    # print(len(voc_ids))
    return torch.LongTensor(voc_ids)
